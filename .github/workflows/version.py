...
def get_lib_version() -> str:
    with request.urlopen("https://crates.io/api/v1/crates/disclude") as response:
...
def update_cargo_toml(self_version: str, lib_version: str):
    with open('Cargo.toml') as f:
        ...
        content = content.replace(
            f'disclude = "{self_version}"', f'disclude = "{lib_version}"')
        content = content.replace(
...
def main():
    ...
    print(f"disclude-rs: {lib_version}, disclude-py: {self_version}")
